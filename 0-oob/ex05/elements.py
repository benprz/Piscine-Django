...
<img src="http://i.imgur.com/pfp3T.jpg" />
    ...
    elem_title = Title("Hello ground!")
    elem_meta = Meta({"charset": "utf-8"})
    elem_head = Head([elem_meta, elem_title], {"lang": "fr"})
    elem_h2 = H2("BURNET Lucille")
    elem_h1 = H1("CV pour la Piscine Django")
    elem_br = Br()
    ...
    elem_li = Li("Algorithmique, Techniques d'optimisation et IA")
    elem_ul = Ul(elem_li)
    elem_ol = Ol(elem_li)
    elem_span = Span([elem_ol, elem_ul])
    elem_th = Th("Langues", {"colspan": "2"})
    elem_tr0 = Tr(elem_th)
    ...
    elem_html = Html([elem_head, elem_body])
    print(elem_html)